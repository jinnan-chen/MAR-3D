...
import mar3d
from mar3d.utils.base import (
    Updateable,
    update_end_if_possible,
    update_if_possible,
)
from mar3d.utils.scheduler import parse_optimizer, parse_scheduler,parse_optimizer_control
from mar3d.utils.config import parse_structured
from mar3d.utils.misc import C, cleanup, get_device, load_module_weights
from mar3d.utils.saving import SaverMixin
...
class BaseSystem(pl.LightningModule, Updateable, SaverMixin):
            ...
            if name in model_state_dict:
                if param.shape == model_state_dict[name].shape:
                    ...
                    print(f"Skipping parameter {name} due to shape mismatch. "
                          ...
                          f"Model shape: {model_state_dict[name].shape}")
            else:
                print(f"Parameter {name} not found in current model")
        
        print(f"Loaded {len(new_state_dict)} parameters with matching shapes.")
        
        # 调用父类的 load_state_dict 方法，使用我们过滤后的状态字典
    ...
    def configure_optimizers(self):
        ...
        # if self.cfg.control:
        #     optim = parse_optimizer_control(self.cfg.optimizer, self)
        # else:
