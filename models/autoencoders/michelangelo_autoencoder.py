...
import mar3d
from mar3d.models.transformers.perceiver_1d import Perceiver
from mar3d.models.transformers.attention import ResidualCrossAttentionBlock
    ...
    def _forward(self, pc, feats):
        ...
        if self.use_multi_reso:
            ...
            if resolution != N:
                ...
                pos = flattened
                ratio = 1.0 * resolution / 2048 # 0.0625
...
class PerceiverCrossAttentionDecoder(nn.Module):
    ...
    def forward(self, queries: torch.FloatTensor, latents: torch.FloatTensor):
        if self.use_checkpoint:
            # 使用 use_reentrant=False 来避免高阶导数的问题
            return checkpoint(self._forward, (queries, latents), self.parameters(), self.use_checkpoint)
            # return checkpoint(self._forward, queries, latents, 
            #                 use_reentrant=False)  # 添加这个参数
        else:
            return self._forward(queries, latents)

@mar3d.register("michelangelo-autoencoder")
class MichelangeloAutoencoder(AutoEncoder):
    ...
    def query(self, 
        ...
        return logits
    def gradient(self,queries,latents):
        queries.requires_grad_(True)
        y=self.query(queries, latents)
        # is_train=True
        ...
        grads = torch.autograd.grad(
            ...
            create_graph=True,  # 确保为 True
            retain_graph=True,  # 确保为 True
            only_inputs=True)[0]
