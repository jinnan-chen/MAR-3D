...
def parse_optimizer_control(config, model):
    # import ipdb
    # ipdb.set_trace()
    params = model.denoiser_model.blocks[0].cross.parameters()
    for num in range(len(model.denoiser_model.blocks)):
        if num>0:
            params=chain(params,model.denoiser_model.blocks[num].cross.parameters())
    ...
    #     ]
    #     mar3d.debug(f"Specify optimizer params: {config.params}")
    # else:
    ...
    if config.name in ["FusedAdam"]:
        ...
        from mar3d.systems import optimizers
