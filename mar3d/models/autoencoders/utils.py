...
from mar3d.utils.base import BaseModule
from mar3d.utils.typing import *
from mar3d.utils.misc import get_world_size
...
class AutoEncoder(BaseModule):
    ...
    def forward(self,
                ...
                surface2: torch.FloatTensor =None,
                surface3: torch.FloatTensor =None,
                queries: torch.FloatTensor =None,
                sample_posterior: bool = True,
                res: torch.FloatTensor =None,
                ):
        shape_latents, kl_embed, posterior = self.encode(surface,surface2,surface3, sample_posterior=sample_posterior)

        # logs=[]
        # de_latents=[]
        # if type(kl_embed) == list:

        #     # for kl_ in kl_embed:

        #     latents_0 = self.decode(kl_embed[0]) # [B, num_latents, width]
        #     latents_1 = self.decode(kl_embed[1])
        #     # import ipdb
        #     # ipdb.set_trace()

        #     latents_0_up=self.cross_level(latents_1,latents_0 )
        #     latents_2=latents_1+latents_0_up

        #     de_latents=[latents_0_up,latents_2]
        #     logits = self.query(queries, latents_0_up) # [B,]
        #     logits_2 = self.query(queries, latents_2) # [B,]
        #     logs=[logits,logits_2]
        # else:
        de_latents = self.decode(kl_embed) # [B, num_latents, width]
        logs = self.query(queries, de_latents) # [B,]


        return kl_embed, de_latents, posterior, logs
